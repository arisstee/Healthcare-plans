...
def run_simulation(db_name="healthcare_simulation.db", num_users=1000):
    ...
    for user_id in range(num_users):
        ...
        # Calculate profitability for each plan for this user
    ...
    conn.commit()
    conn.close()
    print(f"Simulation complete. {len(results)} records inserted into the database.")
    
    # subscription_simulator.py (continued)
import pandas as pd

# Step 6: Analyze the simulation results
def analyze_results(db_name="healthcare_simulation.db"):
    """
    Analyzes the simulation results from the database and prints a summary.
    """
    conn = sqlite3.connect(db_name)
    
    # Use pandas to read the SQL query and perform analysis
    # This is much easier than manual aggregation
    df = pd.read_sql_query("SELECT * FROM simulation_results", conn)
    conn.close()

    print("\n--- Simulation Analysis ---")
    
    # Calculate average profit per plan
    avg_profit_per_plan = df.groupby('plan_name')['profit'].mean().sort_values(ascending=False)
    
    print("\nAverage Monthly Profit Per Plan:")
    print(avg_profit_per_plan.to_string())
    
    # Analyze which plan is best for each user type
    avg_profit_by_user_type = df.groupby(['user_type', 'plan_name'])['profit'].mean().unstack()
    
    print("\nAverage Monthly Profit by User Type and Plan:")
    print(avg_profit_by_user_type.to_string())
...
if __name__ == "__main__":
    # Run the simulation with 1,000 users
